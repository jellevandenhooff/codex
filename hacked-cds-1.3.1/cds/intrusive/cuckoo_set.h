    ...
    namespace cuckoo {
        ...
        */
        ...
        /**
            ...
            Template arguments:
            - \p RecursiveLock - the type of recursive mutex. The default is \p boost::recursive_mutex. The mutex type should be default-constructible.
        ...
        */
        ...
        struct stat {
            ...
            void    onFindEqualSuccess()    { ++m_nFindEqualSuccess; }
            void    onFindEqualFailed()     { ++m_nFindEqualFailed; }
        ...
        struct empty_stat {
            ...
            void    onFindEqualSuccess()    const {}
            void    onFindEqualFailed()     const {}
        ...
        */
        ...
        namespace details {
            ...
            template <typename Predicate, typename NodeTraits, bool Ordered>
            struct contains ;

            template <typename Predicate, typename NodeTraits>
            struct contains<Predicate, NodeTraits, true>
            {
                template <typename BucketEntry, typename Position, typename Q>
            ...
            template <typename Predicate, typename NodeTraits>
            struct contains<Predicate, NodeTraits, false>
            {
                template <typename BucketEntry, typename Position, typename Q>
                static bool find( BucketEntry& probeset, Position& pos, unsigned int nTable, size_t nHash, Q const& val )
                {
            ...
            template <bool Ordered, typename True, typename False>
            struct select_contains_implementation ;

            template <typename True, typename False>
            struct select_contains_implementation< true, True, False>
            {
                typedef True type ;
            };

            template <typename True, typename False>
            struct select_contains_implementation< false, True, False>
            {
                typedef False type ;
            };
...
#   if !defined(CDS_CXX11_LAMBDA_SUPPORT) || (CDS_COMPILER == CDS_COMPILER_MSVC && CDS_COMPILER_VERSION == 1600)
        template <typename Disposer>
...
#   endif
        ...
        typedef typename cuckoo::details::select_contains_implementation< c_isSorted
            , cuckoo::details::contains< key_comparator, node_traits, true >
            , cuckoo::details::contains< key_equal_to, node_traits, false >
    ...
    protected:
        ...
        template <typename Q>
        unsigned int contains( position * arrPos, size_t * arrHash, Q const& val )
        {
            // Buckets must be locked

            for ( unsigned int i = 0; i < c_nArity; ++i ) {
                bucket_entry& probeset = bucket( i, arrHash[i] ) ;
                if ( contains_action::find( probeset, arrPos[i], i, arrHash[i], val ))
                    return i ;
            }
        ...
        bool find_( Q& val, Func f )
        {
            ...
            unsigned int nTable = contains( arrPos, arrHash, val ) ;
        ...
        template <typename Q, typename Equal, typename Func>
        bool find_equal_( Q& val, Equal eq, Func f )
        {
            hash_array arrHash      ;
            hashing( arrHash, val ) ;
            scoped_cell_lock sl( m_MutexPolicy, arrHash ) ;
            ...
            for ( unsigned int i = 0; i < c_nArity; ++i ) {
                ...
                for ( bucket_iterator it = probeset.begin(), itEnd = probeset.end(); it != itEnd; ++it ) {
                    value_type& iv = *node_traits::to_value_ptr( *it ) ;
                    if ( eq( iv, val )) {
                        cds::unref(f)( iv, val ) ;
                        m_Stat.onFindEqualSuccess() ;
                        return true ;
                    }
                }
            }

            m_Stat.onFindEqualFailed() ;
            return false ;
        }
...
#   ifndef CDS_CXX11_LAMBDA_SUPPORT
        ...
        bool find_with_( Q& val, Less pred, Func f )
        {
            ...
            for ( unsigned int i = 0; i < c_nArity; ++i ) {
                ...
                for ( bucket_iterator it = probeset.begin(), itEnd = probeset.end(); it != itEnd; ++it ) {
                    value_type& iv = *node_traits::to_value_ptr( *it ) ;
                    if ( !pred( iv, val ) && !pred( val, iv )) {
                        cds::unref(f)( iv, val ) ;
                        m_Stat.onFindWithSuccess() ;
                        return true ;
                    }
                }
            }

            m_Stat.onFindWithFailed() ;
            return false ;
        }
#   endif
                ...
                while ( true ) {
                    ...
                    while ( i != nTable ) {
                        ...
                        if ( bkt.size() < m_nProbesetThreshold ) {
                            position pos ;
                            contains_action::find( bkt, pos, i, arrHash[i], *pVal ) ; // must return false!
                ...
                for ( unsigned int nTable = 0; nTable < c_nArity; ++nTable ) {
                    ...
                    for ( size_t k = 0; k < nOldCapacity; ++k ) {
                        ...
                        for ( bucket_iterator it = pTable[k].begin(), itEnd = pTable[k].end(); it != itEnd; it = itNext ) {
                            ...
                            contains( arrPos, arrHash, val ) ; // must return c_nUndefTable
            ...
            while (true) {
                {
                    ...
                    if ( contains( arrPos, arrHash, val ) != c_nUndefTable ) {
...
#       endif
        ...
        /**
            The function is an analog of \ref find(Q&, Func) but \p cmp is used for key comparison.
        ...
        */
        ...
        /// Find the key \p val with comparing functor \p cmp
        /**
            The function is an analog of \ref find(Q const&, Func) but \p cmp is used for key comparison.
            \p Compare has the interface like \p std::less.
        */
        template <typename Q, typename Less, typename Func>
        bool find_with( Q const& val, Less pred, Func f )
        {
            //TODO: make equal_to from cmp!!!
#       ifdef CDS_CXX11_LAMBDA_SUPPORT
            return find_equal_( val, [&pred]( value_type const& i, Q const& v){ return !pred(i, v) && !pred(v, i); }, f ) ;
#       else
...
#       endif
        ...
        /// Find the key \p val with equality functor \p eq
        /**
            The function is an analog of \ref find(Q const&, Func) but \p eq is used for key equality testing.
            \p Equal has the interface like \p std::equal_to.
        */
        template <typename Q, typename Equal, typename Func>
        bool find_equal( Q const& val, Equal eq, Func f )
        {
            return find_equal_( val, eq, f ) ;
        }

        /// Find the key \p val with equalty functor \p eq
        /**
            The function is an analog of \ref find(Q const&) but \p eq is used for key equality testing.
            \p Equal has the interface like \p std::equal_to.
        */
        template <typename Q, typename Equal>
        bool find_equal( Q const& val, Equal eq )
        {
#       ifdef CDS_CXX11_LAMBDA_SUPPORT
            return find_equal( val, eq, [](value_type& , Q const& ) {} )    ;
#       else
            return find_equal( val, eq, empty_find_functor() )    ;
#       endif
        }
