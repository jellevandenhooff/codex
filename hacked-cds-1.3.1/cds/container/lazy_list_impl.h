...
#       ifdef CDS_BUG_STATIC_MEMBER_IN_LAMBDA
            ...
            // like as MichaelList::node_to_value that requires to capture *this* despite on node_to_value is static function
            value_type& (* n2v)( node_type& ) = node_to_value ;
            return base_class::find_at( &refHead, val, [&f,n2v](node_type& node, Q& val){ cds::unref(f)( n2v(node), val ); })  ;
#       else
            return base_class::find_at( &refHead, val, [&f](node_type& node, Q& val){ cds::unref(f)( node_to_value(node), val ); })  ;
#       endif
#   else
