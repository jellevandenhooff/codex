...
#   endif
    ...
    public:
        ...
        */
        ...
        /**
            If \p bRaiseException is \p true (that is the default), the function raises an exception gc::too_little_hazard_pointers
        ...
        */
        static bool check_available_guards( size_t nCountNeeded, bool bRaiseException = true )
        {
            if ( hrc::GarbageCollector::instance().getHazardPointerCount() < nCountNeeded ) {
                if ( bRaiseException )
                    throw cds::gc::too_little_hazard_pointers() ;
                return false ;
            }
            return true ;
        }
